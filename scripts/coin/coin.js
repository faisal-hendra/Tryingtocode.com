...
import {
  ...
  setUserDatapoint,
  increaseCoins,
} from "../firebase-backend/firebase.js";
...
let getUserCoins = async () => {
  ...
  if (userData == null || userData.coins == null) {
    return 0;
  }
...
if (canvas) {
  ...
  window.addEventListener("correctCode", (details) => {
    ...
    getCoin(
      ...
      sidebarElement.coinCounter /*, startElement*/,
    );
...
let incrimentDisplayNumber = (amm = 1, startString = "") => {
  let currentCoins = parseInt(
    subtractString(sidebarElement.coinCounter.innerHTML, startString),
  );
