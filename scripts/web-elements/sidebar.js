...
class TTCSidebar extends HTMLElement {
  ...
  render() {
    ...
    if (typeof window.applySettings !== "undefined") {
      window.applySettings();
    }
  }

  findElements() {
    this.coinCounter = this.querySelector(
      "[data-js-tag='sidebar-coin-counter']",
    );
    ...
    this.toggleThisPartOfSidebar = this.querySelector(
      "[data-js-tag='toggle-this-part-of-sidebar']",
    );
    this.toggleSidebarContainer = this.querySelector(
      "[data-js-tag='toggle-sidebar']",
    );
    this.toggleSidebarButton = this.toggleSidebarContainer.querySelector(
      "[data-js-tag='togle-sidebar-button']",
    );
  ...
  setupFunctionality() {
    ...
    this.toggleSidebarImageButton = new ImageButton(
      this.toggleSidebarButton,
      this.toggleSidebarArt,
      1,
    );
  ...
  updateDisplayNumber(updatedNumber, startString) {
    if (typeof updatedNumber !== "number" || typeof startString !== "string") {
      console.error("incorrect datatype sent to display number!");
    }
